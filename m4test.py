...
def load_cached_data():
    ...
    print("[EVENT] Accessing Wintra Data Cache for Model 4 Optimization...")
...
def precalculate_indicators(universe_dict, ihsg_data):
    print("[EVENT] Pre-calculating RS Lines and High Proximity...")
    processed_dict = {}
...
def run_grid_search(universe_dict, start_date, end_date):
    print(f"\n🚀 MODEL 4 OPTIMIZATION: BULL MARKET ALPHA SEARCH ({start_date} to {end_date})")
    print("─"*110)
    
    # ⚙️ ALPHA PARAMETER SPACE
    rs_speeds = [(10, 20), (20, 50)]         # Fast RS vs Structural RS
    vol_reqs = [1.0, 1.2, 1.5]               # Volume Multiplier (1.0 = average volume)
    high_proximity = [2.0, 5.0, 10.0]        # Must be within X% of 20-day high
    
    combinations = list(itertools.product(rs_speeds, vol_reqs, high_proximity))
    total_combs = len(combinations)
    print(f"[EVENT] Testing {total_combs} Alpha permutations...")
    
    results = []
    start_time = time.time()
    
    for idx, params in enumerate(combinations):
        rs_pair, v_mult, prox_lim = params
        f_len, s_len = rs_pair
        sys.stdout.write(f"\r[COMPUTING] {idx+1}/{total_combs} | RS:{f_len}/{s_len} Vol:>{v_mult}x Prox:<{prox_lim}%...")
        sys.stdout.flush()
        ...
        for ticker, df in universe_dict.items():
            ...
            # Calculate dynamic RS MAs based on current loop parameters
            rs_fast = ta.ema(test_df['RS_Line'], length=f_len)
            rs_slow = ta.sma(test_df['RS_Line'], length=s_len)
            
            # Calculate Price MAs to ensure stock isn't crashing while outperforming
            p_fast = ta.ema(test_df['Close'], length=f_len)
            p_slow = ta.sma(test_df['Close'], length=s_len)
            
            # Align boolean masks
            is_rs_trending = (test_df['RS_Line'] > rs_fast) & (rs_fast > rs_slow)
            is_price_trending = (test_df['Close'] > p_fast) & (p_fast > p_slow)
            is_vol_confirmed = test_df['Volume'] > (test_df['Avg_Vol_20'] * v_mult)
            is_near_high = test_df['Dist_to_High'] <= prox_lim
            
            signals = is_rs_trending & is_price_trending & is_vol_confirmed & is_near_high
            signal_dates = test_df[signals].index
            
            for d in signal_dates:
                ret = evaluate_t3_trade(universe_dict[ticker], d) # Evaluate on full dataset
        ...
        if all_returns:
            ...
            results.append({
                'params': {'rs': f"{f_len}/{s_len}", 'vol': v_mult, 'prox': prox_lim},
                'signals': len(all_returns), 'win_rate': win_rate, 'ev': ev
            })

    print(f"\n[EVENT] Search Complete in {time.time() - start_time:.2f}s.")
    
    # Priority: We want elite signals, so we look for counts between 10 and 150
    valid = [r for r in results if 10 <= r['signals'] <= 150]
    sorted_results = sorted(valid, key=lambda x: (x['win_rate'], x['ev']), reverse=True)
    
    print(f"🏆 TOP CONFIGURATIONS FOR ALPHA LEADERSHIP")
    print("─"*110)
    print(f"{'Rank':<5} | {'RS MAs':<10} | {'Vol Mult':<10} | {'Dist to High':<15} | {'Signals':<8} | {'Win Rate':<10} | {'EV'}")
    print("─"*110)
    for i, res in enumerate(sorted_results[:3]):
        p = res['params']
        print(f"#{i+1:<4} | {p['rs']:<10} | > {p['vol']}x{'':<6} | < {p['prox']}%{'':<10} | {res['signals']:<8} | {res['win_rate']:.2f}%{'':<4} | {res['ev']:+.2f}%")
